...
from app.rag.service import RAGError, backfill_user_embeddings, rebuild_monthly_embedding
...
@router.post("/embeddings/backfill", response_model=BackfillEmbeddingsResponse)
def backfill_embeddings_endpoint(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> BackfillEmbeddingsResponse:
    """Rebuild per-expense, daily, and monthly embeddings from the user's full history.

    Run once after enabling RAG on an account with pre-existing expenses,
    or to recover after the embeddings API was unavailable for a while.
    Ongoing writes stay in sync automatically and don't need this.
    """
    result = backfill_user_embeddings(db, user_id=current_user.id)
